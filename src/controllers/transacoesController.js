...
const createTransacao = async (req, res) => {
  ...
  if (!['deposito', 'saque', 'aposta', 'ganho', 'ajuste'].includes(tipo)) {
    res.status(400).json({ error: 'Tipo inválido. Deve ser: deposito, saque, aposta, ganho ou ajuste' });
    return;
  }
  ...
  try {
    ...
    try {
      ...
      if (tipo === 'deposito' || tipo === 'ganho') {
        ...
        saldoAdjustment = valor; // Ajuste pode ser positivo ou negativo
      }
    ...
    try {
      ...
      if (transacao.casa_id) {
        ...
        if (transacao.tipo === 'deposito' || transacao.tipo === 'ganho') {
          ...
          saldoAdjustment = -transacao.valor; // revertendo o ajuste
        }
